...
import os, fnmatch, re, shutil, errno, shlex
...
def set_params_folders(tab: list, params: dict):
...
def set_params(tab: list, params: dict):
...
def main():
    '''Main function'''
    set_params(_argv, params)
    set_params_folders(_argv, params)
    if params["help"]:
        print_help(_argv[0])
...
def get_override_params(header_comments):
    header_lines = header_comments.splitlines()
    for header_line in header_lines:
        trimmed = header_line[1:].strip()
        if trimmed.startswith("update_translations"):
            return set_params(shlex.split(trimmed), {})
    return {}
...
def strings_to_text(dkeyStrings, dOld, mod_name, header_comments):
    ...
    if header_comments is not None:
        ...
        params_override = get_override_params(header_comments)
